...
const outputDir = path.join(process.cwd(), "public", "ogs");
...
async function main() {
  ...
  const slugToRegenerate = shouldRegenerateSlug
    ? option.split("--slug=")[1]
    : null;
  ...
  themes.forEach(async (theme) => {
    ...
    if (
      ...
      doesFileExist(filename, outputDir)
    ) {
      return;
    }
    ...
    const url = `http://localhost:3000/og?${queryParams}`;
